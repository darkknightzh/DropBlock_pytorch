...
class DropBlock2D(nn.Module):
    ...
    def forward(self, input):
        ...
        Msum = F.conv2d(M, torch.ones((input.shape[1], 1, self.block_size, self.block_size)).to(device=input.device, dtype=input.dtype),
...
class DropBlock2DV2(nn.Module):
    def __init__(self, keep_prob=0.9, block_size=7):
        super(DropBlock2DV2, self).__init__()
        self.keep_prob = keep_prob
        self.block_size = block_size
    ...
    def forward(self, input):
        ...
        Msum = F.max_pool2d(M, kernel_size=[self.block_size, self.block_size], stride=1, padding=self.block_size // 2)
